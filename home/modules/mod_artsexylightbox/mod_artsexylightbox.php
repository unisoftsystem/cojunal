...
if ($color == 'white' || $color == 'black') {
  $document->addScript(JURI::root() . 'modules/mod_artsexylightbox/artsexylightbox/js/script.v2.2.js');
  ...
  $document->addScript(JURI::root() . 'modules/mod_artsexylightbox/artsexylightbox/js/script.v2.3.4.js');
...
$jsCode .= '<script type="text/javascript">asljQuery(function(){asljQuery(document).ready(function(){';
  ...
  while (list ($key, $entry) = each ($filelist)) {
    if ($entry != '.' && $entry != '..' && aslMisImage($path . $entry)) {
      ...
      if ($singleImage && $o == 0 && $showSingleImage != 'false') {
        ...
        $imagesCode .= "' data-rel='sexylightbox[". $id ."]' class='artsexylightboxpreview' ";
      } else {
