...
class AdminBaseController extends GxController {
	...
	public function actionGetSearchClientes()
    {
      $model = new AdvisersBase;
      if(isset($_POST['busqueda'])){
        $this->apiResponse([$model::search_clientes($_POST['busqueda'])]);
      }else {
        $this->apiResponse(false);
      }
	}
	...
	public function actionlistapaymentsbywallets($idwallets)
	{
		$model = new AdvisersBase;
		$idwallet = $_GET['idwallets'];
		if(isset($idwallet)){
			$this->apiResponse([$model::wallets_payment($idwallet)]);
		}else {
			$this->apiResponse(false);
		}
}
...
public function actionGetpagosrealizados() {
	$model = new AdvisersBase;
