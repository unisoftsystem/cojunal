<template>
  <div class="search">
    ...
    <DataTable v-if="idCampaing" :endPoint="endPoint" :headings="headings" :selectRow="selectRow" :params="{idAdviser: idCampaing}" :search="false">
      ...
      <template slot="acciones" slot-scope="props">
        <a class="btn-link" @click="goTo('/coordinador/asignar-tarea/'+props.slotScope.id+'/'+props.slotScope.idAdviser, {
          idWallet: props.slotScope.id,
          idAdviser: props.slotScope.idAdviser
        })">Asignar tarea</a>
      </template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        endPoint: 'webapi/GetCampanasPorAsesor',
        headings: {
          campana: {
            title: 'Campaña',
          },
